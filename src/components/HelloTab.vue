  ...
  <div class="tabs-container">
    <div class="tabs">
      <ul>
        <li
          v-for="tab in tabs"
          v-bind:key="tab.id"
          :class="{ 'is-active': tab.name === selectedTab.name}"
        >
    ...
    <div v-if="selectedTab.name ==='Topics'" class="tabs-topics-details">
      <div v-for="item in topics" v-bind:key="item.title">
        ...
        <span class="topics-posts">
          <animated-number :value="item.posts" :duration="900" />POSTS
        </span>
      </div>
    </div>
  ...
  computed: {
    archives: () => {
      return Array.from(Array(10), (x, index) =>
        "Grumpy Cat Contribute Nr.".concat(index)
      );
    },
    topics: () => {
      const list = [
      ...
      return list.map(title => {
        return { title, posts: Math.floor(Math.random() * 100) };
      });
    }
  },
  methods: {
